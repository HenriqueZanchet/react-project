...
export default function () {
    ...
    return <>
        {loading
            ? <input id="input-cidade" placeholder="cidade"></input>
            : <select id="select-cidade" onChange={CidadeSelecionada} value={cidade}>{cidades.map(({ nome }, idx) => <option key={idx} value={nome}>{nome}</option>)}</select>
        }
    </>
}